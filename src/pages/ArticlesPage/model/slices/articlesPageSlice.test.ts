...
describe('articlesPageSlice', () => {
  test('test set views', () => {
    ...
    expect(articlesPageReducer(state as ArticlesPageSchema, articlesPageActions.setView(ArticlesViews.Big))).toEqual({
  ...
  test('test set  number order', () => {
    ...
    expect(articlesPageReducer(state as ArticlesPageSchema, articlesPageActions.setOrder('desc'))).toEqual({
      order: 'desc',
    });
  });
  test('test set  number search', () => {
    const state: DeepPartial<ArticlesPageSchema> = { search: 'rrr' };
    expect(articlesPageReducer(state as ArticlesPageSchema, articlesPageActions.setSearch('aaa'))).toEqual({
      search: 'aaa',
    });
  });
  test('test set  number sort', () => {
    const state: DeepPartial<ArticlesPageSchema> = { sort: ArticleSortField.Created };
    expect(articlesPageReducer(state as ArticlesPageSchema, articlesPageActions.setSort(ArticleSortField.Views))).toEqual({
      sort: ArticleSortField.Views,
    });
  });
  test('test set  number type', () => {
    const state: DeepPartial<ArticlesPageSchema> = { type: ArticleType.Economics };
    expect(articlesPageReducer(state as ArticlesPageSchema, articlesPageActions.setType(ArticleType.IT))).toEqual({
      type: ArticleType.IT,
    });
  });
  ...
  test('test  loading articles service pending', () => {
    ...
    expect(articlesPageReducer(state as ArticlesPageSchema, fetchArticlesList.pending('', { replace: true }))).toEqual({
      isLoading: true,
      error: undefined,
      ids: [],
      entities: {},
  ...
  test('test  loading articles service fullfield', () => {
    ...
    expect(articlesPageReducer(state as ArticlesPageSchema, fetchArticlesList.fulfilled(articles, '', { replace: true }))).toEqual({
      isLoading: false,
      hasMore: false,
      ids: ['1', '2'],
      entities: {
